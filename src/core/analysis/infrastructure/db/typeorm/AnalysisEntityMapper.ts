...
import { AnalysisLiming, AnalysisLimingId } from "../../../../analysis/domain/AnalysisLiming";
import { AnalysisNpk, AnalysisNpkId } from "../../../../analysis/domain/AnalysisNpk";
...
export class AnalysisEntityMapper {
  static toTypeEntity(analysis: Analysis): AnalysisEntity {
    ...
    if (typeAnalysis instanceof AnalysisLiming) {
      ...
      liming.desiredBaseSaturation = typeAnalysis.getDesiredBaseSaturation().getValue;
      liming.currentBaseSaturation = typeAnalysis.getCurrentBaseSaturation().getValue;
      liming.totalCationExchangeCapacity = typeAnalysis.getTotalCationExchangeCapacity().getValue;
      liming.relativeTotalNeutralizingPower = typeAnalysis.getRelativeTotalNeutralizingPower().getValue;
    ...
    if (typeAnalysis instanceof AnalysisNpk) {
      ...
      npk.expectedProductivity = typeAnalysis.getExpectedProductivity().getValue;
...
if (entity.typeAnalysis === "LIMING" && entity.typeAnalysis) {
      typeAnalysis = new AnalysisLiming({
        ...
        desiredBaseSaturation: new DesiredBaseSaturation(entity.liming.desiredBaseSaturation),
        currentBaseSaturation: new CurrentBaseSaturation(entity.liming.currentBaseSaturation),
        totalCationExchangeCapacity: new TotalCationExchangeCapacity(entity.liming.totalCationExchangeCapacity),
        relativeTotalNeutralizingPower: new RelativeTotalNeutralizingPower(entity.liming.relativeTotalNeutralizingPower),
    ...
    if (entity.typeAnalysis === "NPK" && entity.npk) {
      typeAnalysis = new AnalysisNpk({
        ...
        expectedProductivity: new ExpectedProductivity(entity.npk.expectedProductivity),
